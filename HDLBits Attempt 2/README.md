...
- **93 Done** and **85 Pending**.
- All **178 problem names** link to their matching HDLBits problem page.
- Among the 93 completed entries, **56** say `No questions asked` and **37** link to a relevant question or review document.
- Every entry from **1 through 93** is now reached in Attempt 2. Entries 86-93 were updated from Kapil's completion confirmation and the eight matching HDLBits tabs open in Chrome on 2026-09-03; entries 94-178 remain Pending.
- The newly reached question-bearing entries are split cleanly: entry 90 has its own standalone FSM deep dive, while entries 87 and 89 share a separate non-FSM Q&A PDF.
...
These groups account for all **37 tracker entries** whose Notes / Discussion cell links to a document. All 37 are now completed entries, and every such cell is a direct hyperlink to the named document.

## Verification record

- All 178 stored HDLBits problem targets were fetched successfully during this checkpoint and returned HTTP 200.
- The eight PDFs contain **88 pages** in total. Every new Day 6 page was text-extracted and freshly rendered; no blank, clipped, overlapping, or unreadable page was found.
...
- The tracker contains **215 Excel `HYPERLINK` formulas**: 178 problem links and 37 document/review links. All retain friendly cached labels.
- Entries 1-85 retain their earlier saved-submission evidence. Entries 86-93 are marked Done from Kapil's current-pass completion confirmation, corroborated by the matching set of eight open HDLBits problem tabs captured on 2026-09-03; no fabricated per-entry timestamps are claimed.

## Evidence boundary

The HDLBits website is the authority for platform acceptance. Saved-submission history supports entries 1-85; Kapil's explicit current-pass completion report supports entries 86-93, with the matching Chrome tabs preserved as evidence. Local simulation is a separate check and was run only where source or a reusable testbench is preserved in this directory; it is not claimed as a local rerun of all accepted submissions. Entries 94 through 178 remain Pending without any current-window submission claim.
