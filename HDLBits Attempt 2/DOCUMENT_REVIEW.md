...
## Ten-day consolidation and question index - 9 September 2026

Kapil selected **10 reading days**, with all the later K-map/Boolean, timer, LFSR and Conway material together on Day 10. The primary [reading index](DOCUMENT_INDEX.md) and PDF contents now lead with specific questions, not just topic headings. The tracker's original day labels and dated completion evidence are unchanged; a later reading day can revisit an earlier tracker entry.

The primary set is **10 PDFs / 166 pages** under `output/pdf/`: all **148 original technical pages**, **3 new LFSR reference pages** and **15 replacement cover/index pages**. Every original technical page appears exactly once. The original 16 PDFs remain unchanged at their existing paths for historical links and reproducible source-page preservation; they are not a second primary reading list. Entry 155 is still explained in both its earlier cleanup and the connected timer series. The basic one-hot exercise, entry 175, remains excluded from explanatory notes as requested.

All 166 pages were rendered with Poppler and visually inspected, including question contents, code, diagrams, tables and the new reference section. Changed pages were rendered again and rechecked. The consolidation keeps the established white/navy presentation, blue links and meaningful diagram colors. It replaces old covers and footer counters, changes the later running headers to Day 10, and updates prose references affected by the merge. A table-cell reference edit was narrowed after visual inspection so the other cells in that row remain intact.

The saved-file checks pass for **253 bookmarks, 673 PDF links (internal and external combined), 109 local index links and all 58 tracker-entry mappings**. Every contents row has its expected destination, every page has exactly one Contents and one All days link, and printed page numbers match physical PDF pages. Fonts are embedded. Source text is preserved apart from the individually recorded reference/header/footer edits; original image bytes and page geometry are unchanged. These are saved-PDF destination and rendered-layout checks, not a new interactive browser PDF-click test. GitHub previews can ignore page fragments; use a downloaded PDF's contents or bookmarks in a supporting reader.

Day 10 pages **20-22** retain the detailed state/next_state explanation and decision table. Pages **43-45** add the requested ending reference: LFSR definition, state, seed, taps, GF(2), period, lockup, Fibonacci/Galois forms, bit-numbering conventions, applications, limits and the shared clocked-state rule. Primary references are Philip Koopman's right-shift LFSR tables and Peter Alfke's Xilinx XAPP052; the text distinguishes XOR zero lockup from the conventional XNOR all-ones case. Checks cover every five-bit state for mask equivalence, all 1,024 pairs for linearity, the deliberately bad five-state tap example, complete 31- and seven-state cycles, the 32-bit first transition `00000001 -> 80200003`, and the saved primitive-order calculation. The 32-bit period is supported algebraically, not by claiming to simulate four billion edges.

The tracker was updated through Artifact Tool-authored labels and a narrow native-hyperlink pass. All **58 discussion labels and destinations** now point to the day-wise answer pages. It remains **178 Done / 0 Pending**, with **236 native hyperlinks**. A before/after package comparison permits changes only to those discussion-cell payloads and hyperlink relationship targets; all other cells, styles, day labels, validations, panes, dimensions and workbook ZIP parts are preserved. The saved workbook passed its error scan and its beginning, middle and end views were visually inspected.

The build manifest records all source-to-final page mappings and deliberate reference edits. Rebuild, verification, rendering and tracker-maintenance commands are documented in [internal/Documentation](internal/Documentation/README.md). The checkpoints below describe the earlier source-note collection and remain historical evidence.
