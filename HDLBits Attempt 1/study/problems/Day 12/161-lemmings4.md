...
|---|---|
| Day | Day 12 |
| Last successful submission | 2026-07-05 1:10:55 PM |
| HDLBits ID | `lemmings4` |
| Attempts | 5 total: 2 successes, 1 incorrect, 2 compile errors, 0 simulation errors |
| Success rate | 40% |
| Source | [HDLBits](https://hdlbits.01xz.net/wiki/lemmings4) |
| Files | [Screenshot](../../images/Day%2012/161-lemmings4-question-and-successful-submission.png) · [Verilog](../../solutions/Day%2012/161-lemmings4.sv) |
...
Lemmings 4 combines the direction/digging FSM with a fall-duration datapath. `willCount` advances only in ordinary walking-fall states. When ground returns, the FSM tests whether the fall exceeded the survival limit. `splatter` is absorbing, so all activity outputs remain low forever afterward.
...
- FSM state selects the phase; a counter measures duration.
- Saturate duration counters when only a threshold matters.
- Terminal states should be explicit and absorbing.
