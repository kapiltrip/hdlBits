...
The DUT output is a wire because the T flip-flop drives it. Clock, reset, and T are registers because the testbench drives them procedurally. Keeping the free-running clock and one-time stimulus in separate \`initial\` processes is intentional concurrency: both timelines advance together under the simulator's event scheduler.`,
  "exams/2014_q3fsm": `This FSM waits in state A for s, then remains in state B while processing w in non-overlapping groups of three clock cycles. The FSM state stores the phase, while sample_count and ones_count are clocked datapath registers that remember progress within the active group.

On the third sampling edge, the decision must include the current w because nonblocking assignments read the old ones_count value. The successful implementation calculates z from ones_count+w, clears both counters, and begins the next group without leaving B.`,
  "exams/2014_q3bfsm": `The state table describes a complete five-state Moore machine. A clocked state register applies synchronous reset and stores next_state, a combinational case statement translates every legal state/input pair from the table, and z is decoded only from registered states S3 and S4.

The default case recovers from the unused three-bit encodings. Keeping these three responsibilities separate makes reset timing, transition logic, and Moore output timing directly auditable.`,
  "exams/2014_q3c": `This exercise requests only Y0, the least-significant bit of the next-state word, and the Moore output z. Splitting the table by x shows that Y0 equals y0 OR y2 when x is zero and NOT y0 AND NOT y2 when x is one.

The two cases are combined with x as the selector. Output z is derived separately from the present-state column and is high only for states 011 and 100. The supplied clk is intentionally unused because no state storage is requested.`,
